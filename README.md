...
| --- | --- | --- | --- |
| Synthetic roster and configurable demand inputs, with no precomputed assignments | `src/lib/data/harbourPineScenario.ts`, `src/lib/scenario.ts`, `src/components/SchedulerWorkbench.tsx` | Fresh fixture and changed-input cases in `tests/unit/scheduling.test.ts` | Change a staff member's maximum hours in **Configure input**, then solve |
...
| Session-only browser state | `src/components/SchedulerWorkbench.tsx` | Release check in `docs/runbook.md` | Run or edit a schedule; **Reset demo** and page reload both return to defaults |
...
The application sends the current, caller-supplied typed scheduling model to its own validated Next.js route. The bundled scenario is only a reproducible starting point; changing the maximum-hours control changes the actual solver input. Results and manual edits live only in the current page session; neither the route nor the browser persists them. Reloading or closing the page discards the current schedule. There is no server-side database, account, cross-device sync, or backup. Do not enter real employee or sensitive personal data.
